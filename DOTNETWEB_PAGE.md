...
## Basic connectivity: Crate can use any PostgreSQL-compatible .NET driver.

An example connection string using Npgsql is:

```csharp
Host=<host>;Port=5432;Database=crate;Username=<user>;Password=<password>;SSL Mode=<sslmode>
```

## Example: Using .NET against our German Weather Data data set

If you want to see a simple example CrateDB/Npgsql application check out this repository:

https://github.com/crate/cratedb-explore/tree/main/src_weather/main/dotnet

This is a C#/.NET load generator that connects to a CrateDB cluster over the PostgreSQL wire protocol and runs a configurable mix of queries against German climate and region data. It assumes you have [loaded the data](https://cratedb.com/explore/iot-analytics/import?use-case=iot).

The single application file is [Program.cs](https://github.com/crate/cratedb-explore/blob/main/src_weather/main/dotnet/Program.cs).

###  What it does

* Opens an Npgsql connection to a CrateDB cluster.
